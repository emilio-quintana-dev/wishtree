...
const SignUp = ({ handleLogin, handleLogout, loggedInStatus, user }) => {
  ...
  const [showConfirmationPassword, setShowConfirmationPassword] =
    useState(false);
  const [isGifter, setIsGifter] = useState(true);
  ...
  const handleSuccesfulAuth = (data) => {
    ...
    window.location.replace(
      `${import.meta.env.VITE_API_ENDPOINT}/${data.user.username}`,
    );
  };
  ...
  const handleSubmit = (event) => {
    ...
    axios
      .post(
        `${import.meta.env.VITE_API_ENDPOINT}/registrations`,
        {
          email,
          password,
          firstName,
    ...
    >
      ...
      >
        ...
        <Box component="form" onSubmit={handleSubmit} noValidate sx={{ mt: 1 }}>
          ...
          <ChunkyButton color={theme.palette.primary.main} width="100%">
            Create Account
          </ChunkyButton>
        </Box>
      </Box>
